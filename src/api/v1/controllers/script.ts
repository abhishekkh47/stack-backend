...
import { UserDBService, zohoCrmService, DeviceTokenService, ScriptService } from "../../../services";
...
class ScriptController extends BaseController {
  ...
  public async kycApproveStaging(ctx: any) {
    ...
    const kycApprove = await ScriptService.sandboxApproveKYC(user, ctx);

    if (kycApprove.status !== 200) return this.BadRequest(ctx, { status: kycApprove, message: kycApprove.response });
