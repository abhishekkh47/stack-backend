    ...
    if (users.length > 0) {
      ...
      for await (let user of users) {
        if (user.type == EUserType.SELF) {
          accountIdDetails = await user.self.userId.toString() == user._id.toString() && user.self
        } else {
        ...
        if (selectedDate <= todayDate) {
          const id  =   user.type == EUserType.SELF ? accountIdDetails.userId : accountIdDetails.childId
          ...
          let contributionRequest = {
            ...
            attributes: {
              "account-id": accountIdDetails.accountId,
              "contact-id": user.type == EUserType.SELF ? user.self.contactId : user.parentChild.contactId,
              "funds-transfer-method": {
                "funds-transfer-type": "ach",
                "ach-check-type": "personal",
                "contact-id": user.type == EUserType.SELF ? user.self.contactId : user.parentChild.contactId,
          ...
          if (contributions.status == 400) {
            ...
            if (deviceTokenData) {
              ...
              await Notification.create({
                title: notificationRequest.title,
                userId: user.type == EUserType.SELF
                ? user.self.userId
                : user.parentChild.userId,
                message: null,
                isRead: ERead.UNREAD,
